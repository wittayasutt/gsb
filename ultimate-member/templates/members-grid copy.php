...
<div class="row">
	...
	<div class="col m3 s12">
		<a href="<?php echo um_user_profile_url(); ?>" title="<?php echo esc_attr(um_user('display_name')); ?>">
			<div class="member z-depth-1">
				<?php if ($profile_photo) {
				$default_size = str_replace( 'px', '', um_get_option('profile_photosize') );
				$corner = um_get_option('profile_photocorner');
				?>
				<div class="um-member-photo radius-<?php echo $corner; ?>"><?php echo get_avatar( um_user('ID'), $default_size ); ?></div>
				<?php } ?>
	...
	<div class="um-member col m3 um-role-<?php echo um_user('role'); ?> <?php echo um_user('account_status'); ?> <?php if ($cover_photos) { echo 'with-cover'; } ?>">
		...
		<?php } ?>
					...
					<div class="um-member-card <?php if (!$profile_photo) { echo 'no-photo'; } ?>">
						...
						<?php if ( $show_name ) { ?>
						<div class="um-member-name"><a href="<?php echo um_user_profile_url(); ?>" title="<?php echo esc_attr(um_user('display_name')); ?>"><?php echo um_user('comname', 'html'); ?></a></div>
						<?php } ?>
						<?php echo um_user('first_name', 'html'); ?>
						<?php echo um_user('comdesc', 'html'); ?>
