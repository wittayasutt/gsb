			...
			<div class="video_nav _mobile">
				...
				<?php
						...
						echo '<select id="select_video_cat">';
							echo '<option value="video">' . 'ทั้งหมด' . '</option>';
...
jQuery(document).ready(function($) {
	$('#select_video_cat').val('<?php $term = get_queried_object(); echo $term->slug;?>');
		$('#select_video_cat').on('change', function() {
			var	selects_active = this.value;
				window.location.href = '<?php echo get_home_url(); ?>'+'/category/'+selects_active;
		});
})
</script>
<?php get_footer(); ?>
